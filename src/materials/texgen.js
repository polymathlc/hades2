...
export function warp(src, n, o = {}) {
  ...
  const wx = o.wx || resampleTo(fbm(wr, { freq, octaves: oct, seed, type: 'grad', ppc: 4 }), wr, n);
  const wy = o.wy || resampleTo(fbm(wr, { freq, octaves: oct, seed: seed + 3331, type: 'grad', ppc: 4 }), wr, n);
  const out = new Float32Array(n * n);
  for (let y = 0; y < n; y++) {
    const row = y * n;
    for (let x = 0; x < n; x++) {
      const i = row + x;
      const sx = x + (wx[i] - 0.5) * 2 * amp, sy = y + (wy[i] - 0.5) * 2 * amp;
...
export function strokes(dst, n, o = {}) {
  ...
  const angAt = (x, y) => (constAngle !== null ? constAngle : sampleWrapAngle(flow, n, x, y));
  ...
  for (let s = 0; s < count; s++) {
    ...
    if (mask) {
      const m = sampleWrap(mask, n, px, py);
      if (rng() > m) continue;
    }
    ...
    for (let t = 0; t <= L; t++) {
      ...
      const prof = Math.pow(Math.sin(Math.PI * Math.pow(k, 0.62)), taper * 0.62);
      ...
      for (let di = -steps; di <= steps; di++) {
        const d = (di / steps) * ww;
        const q = 1 - Math.abs(d) / ww;
        let f = Math.pow(q, softness);
        if (bristle > 0) f *= 1 - bristle * 0.5 * (1 + Math.cos(d * bfreq + bphase)) * 0.5;
...
export function normalTexture(h, n, strength = 2, o = {}) {
  const t = new THREE.DataTexture(heightToNormal(h, n, strength), n, n, THREE.RGBAFormat);
...
// ---------------------------------------------------------------------------
// Seam verification (used by the dev harness; cheap enough to keep shipped)
// ---------------------------------------------------------------------------
/**
 * Measures wrap discontinuity: mean |f(0,y)-f(n-1,y)| compared against the mean
 * interior gradient. < ~1.2 means the seam is indistinguishable from ordinary
 * internal variation, i.e. the texture tiles.
 */
export function seamError(f, n) {
  let edge = 0, inner = 0, ec = 0, ic = 0;
  for (let y = 0; y < n; y++) { edge += Math.abs(f[y * n] - f[y * n + n - 1]); ec++; }
  for (let x = 0; x < n; x++) { edge += Math.abs(f[x] - f[(n - 1) * n + x]); ec++; }
  // mean interior neighbour delta over the whole field (robust reference)
  for (let y = 0; y < n; y += 3) {
    const row = y * n;
    for (let x = 0; x < n - 1; x += 3) { inner += Math.abs(f[row + x] - f[row + x + 1]); ic++; }
  }
  return (edge / Math.max(1, ec)) / Math.max(1e-6, inner / Math.max(1, ic));
}

export default {
  makeRng, fbm, ridged, turbulence, warp, warp2, fbmWarped, worleyField, cracks,
  blurWrap, cavityMask, edgeMask, dirtMask, flowField, strokes, strokesRGB,
  lut, applyRamp, applyRamp2, compositeRamp, shadeRGB, tintRGB,
  heightToNormal, aoFromHeight, artisticRoughness,
  drawDisc, drawLine, drawArc, drawPolyline, drawRect,
  meanderBand, palmette, guilloche, beadRow, laurelBand,
  ashlar, tileGrid, weave, woodGrain, marbleVeins, veinNetwork,
  rgbTexture, normalTexture, ormTexture, fieldTexture, seamError,
};
